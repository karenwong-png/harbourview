...
RULES
- Read the floor plans carefully. Count only LETTABLE rooms (bedrooms) — never count kitchen, dining, living, bathrooms, balcony, A/C yard, or store as rooms. Treat a "+1" (e.g. 1+1, 3+1) as a STUDY/flexi, not a full bedroom, unless it clearly fits a bed + door. Classify each room (master / ensuite / queen / standard / small / partition). Trace each bathroom door: an arc swinging INTO a bedroom = ensuite (premium); into the corridor = common. A cluster of two baths often means ONE is ensuite, not both. Estimate built-up from the mm dimensions if shown; call it an estimate.
- Assess the LIVING HALL for conversion (the biggest income lever): the hall is usually the largest space and can often be partitioned into 1-2 extra bedrooms (~one room per 80-110 sq ft), added to the Optimized scenario as extra rooms. Price them by quality: windowed/balcony hall = a proper room; internal hall = lowest tier. Never block the front door / escape path. TAG these and partition rooms, and ALWAYS flag: Optimized assumes partitioning/hall-conversion is permitted (strata by-laws, fire egress, council) — mark informational-only for PR1MA/no-partition condos.
